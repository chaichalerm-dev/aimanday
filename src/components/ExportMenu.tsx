...
function MenuItem({
  ...
  return (
    <button
      onClick={onClick}
      className="flex w-full items-center gap-3 rounded-[4px] px-3 py-2 text-left text-sm text-[var(--ink)] hover:bg-[var(--paper-muted)]"
    >
  ...
  return (
    <>
      ...
      <button
        ref={triggerRef}
        onClick={() => setOpen(o => !o)}
        className={`flex items-center gap-1.5 rounded-[4px] border px-3 py-1.5 text-xs font-medium ${
          open
      ...
      {mounted && open && createPortal(
        <div
          ref={menuRef}
          className="ui-panel fixed z-[200] p-2 shadow-lg"
          style={{
            top: coords.top,
            left: coords.left,
            width: MENU_WIDTH,
